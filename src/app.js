...
/* Yield to the browser so the "Working…" overlay can paint before a long
   computation starts. requestAnimationFrame is throttled to a standstill in a
   background tab, so a plain rAF await would never resolve and the overlay
   would hang forever if the user switched tabs — hence the timeout race. */
...
/* ============================================== INLINE HELP (the ? chips) == */
/* Short, plain answers to "what is this and why would I touch it". Attached to
   a control with helpChip("key"); one delegated listener serves them all. */
const HELP = {
  mode: `<b>How each row is read.</b> If a row already names two people who are connected, that is an
    <b>edge list</b>. If a row says "this person took part in this thing", that is a
    <b>co-occurrence list</b> — everyone sharing a document, meeting or event gets linked, and the edge
    weight counts how many they share. Most spreadsheets in the humanities are the second kind.`,
  weight: `<b>How strong a tie is.</b> The default counts how often the two appear together. Choosing a
    number column instead sums that column. Weight drives edge thickness, the threshold slider, and
    (optionally) the distance used by betweenness and closeness.`,
  agg: `<b>When the same pair appears twice.</b> Two rows linking the same two people are merged into one
    edge. This decides whether the weights add up, or the largest wins, or they are averaged.`,
  directed: `<b>Does direction matter?</b> Tick this only if A→B means something different from B→A —
    a letter sent, a payment made, a citation. Kinship, co-signing and co-appearance are not directed.`,
  nodeinfo: `<b>What you see when hovering a node.</b> Pick the columns worth reading at a glance. When a
    person has several different values in a column, the most frequent one is shown.`,
  edgeinfo: `<b>What you see when hovering a link.</b> In co-occurrence mode this is where the shared
    documents or events are listed — usually the most useful thing on the screen.`,
  ...
  components: `<b>Pieces that never touch.</b> Most real networks are not one connected lump. Narrowing the
    ...
    everything else.`,
  threshold: `<b>The most useful control here.</b> Hides ties below a weight. On a co-occurrence network,
    where every document is already a complete clique, raising this until the picture becomes readable is
    usually the difference between a hairball and a finding.`,
  colour: `<b>Colour carries a category.</b> Any attribute with a manageable number of values can be used.
    Clicking a row in the legend hides that value, which is a fast way to ask "what does this network look
    like without them?"`,
  size: `<b>Size carries a number.</b> Metrics appear in this list once you compute them in the Analysis
    tab. Size is the quickest way to see whether the busy nodes and the structurally important nodes are
    the same people — they often are not.`,
  labels: `<b>Names on the canvas.</b> Showing all of them is unreadable above about fifty nodes, so the
    default labels only the most central. Labels that would overlap one already drawn are dropped.`,
  weighted: `<b>Should weight count as distance?</b> With this on, a heavier tie is treated as a
    <i>shorter</i> distance (1 ÷ weight) when betweenness and closeness look for shortest paths — the
    normal reading for co-occurrence data, where a stronger tie means the two are closer.`,
  overview: `<b>The shape of the whole network.</b> Density is the share of possible links that exist.
    Path length and diameter are counted in hops inside the largest component. Degree assortativity is
    positive when hubs attach to hubs, negative when hubs attach to the periphery.`,
  ranking: `<b>Everyone, sorted.</b> Click a row to fly to that node; shift-click to mark it for export.
    Reading two different metrics down this list side by side is where most findings start.`,
  marked: `<b>A group you carved out by hand.</b> Shift-drag a box on the canvas, or shift-click nodes one
    at a time. Once marked you can grow the group to its neighbours, isolate it, and download its nodes and
    its internal edges — that edge file can be re-uploaded here to analyse the subgroup on its own.`,
  picture: `<b>Exports exactly what is on screen</b> — same zoom, same filters, same labels. Arrange the
    view first. SVG stays sharp at any size and can be edited in Illustrator or Inkscape, which is what you
    want for a figure in a paper; PNG is a flat image for slides.`,
  session: `<b>Reproducibility.</b> Stores the column mapping, every setting and the exact node positions.
    Re-opening it rebuilds this precise figure — worth saving next to any picture you publish.`
};
function helpChip(k) {
  return `<span class="help" data-help="${k}" tabindex="0" role="button" aria-label="What is this?">?</span>`;
}
(function wireHelp() {
  const pop = $("#helppop");
  let hideTimer = null;
  const show = (el) => {
    const txt = HELP[el.dataset.help];
    if (!txt) return;
    ...
    if (left + p.width > innerWidth - 10) left = Math.max(10, r.left - p.width - 10);
    if (top + p.height > innerHeight - 10) top = Math.max(10, innerHeight - p.height - 10);
    pop.style.left = left + "px";
    pop.style.top = top + "px";
  };
  const hide = () => { hideTimer = setTimeout(() => pop.style.display = "none", 120); };
  document.addEventListener("mouseover", e => {
    const el = e.target.closest?.(".help");
    if (el) show(el);
  });
  document.addEventListener("mouseout", e => { if (e.target.closest?.(".help")) hide(); });
  document.addEventListener("focusin", e => { if (e.target.closest?.(".help")) show(e.target); });
  document.addEventListener("focusout", e => { if (e.target.closest?.(".help")) hide(); });
...
function renderDataPanel() {
  ...
  <div class="sect">
    <h3>3 \u00b7 Node attributes <span class="pill">optional</span>${helpChip("attrs")}</h3>
...
function openMapper() {
  ...
  openModal("Map your columns", `
    ...
    <div class="sect">
      <h3>How should each row be read?${helpChip("mode")}</h3>
      <div class="grid2">
    ...
    <div class="sect">
      ...
      <div class="grid2">
        <div class="row">
          <label>Edge weight${helpChip("weight")}</label>
          <select id="m-w">
        ...
        <div class="row">
          <label>If the same pair repeats${helpChip("agg")}</label>
          <select id="m-agg">
      ...
      <div class="row" id="dir-row" style="${mode === "co" ? "display:none" : ""}">
        <label class="chk"><input type="checkbox" id="m-dir" ${prev.directed ? "checked" : ""}>
          <span>Directed — A \u2192 B means something different from B \u2192 A (arrows are drawn, and metrics respect direction)</span></label>${helpChip("directed")}
      </div>
    </div>
    ...
    <div class="sect">
      ...
      <div class="grid2">
        <div class="row">
          <label>Node details${helpChip("nodeinfo")}</label>
        ...
        <div class="row">
          <label>Edge details${helpChip("edgeinfo")}</label>
...
function build() {
  ...
  // A weight column that turns out not to be numeric would silently make every
  // edge weigh zero — count the failures so we can say so instead.
  let badWeights = 0;
  const wOf = row => {
    if (m.weightCol === "__count__") return 1;
    const v = parseFloat(row[m.weightCol]);
    if (!Number.isFinite(v)) { badWeights++; return 0; }
    return v;
  };
  ...
  // the previous network's metrics and component numbers are gone; anything
  // pointing at them would silently render grey or filter to nothing
  if (S.view.colorBy === "__community__") S.view.colorBy = "__none__";
  if (!["__uniform__", "degree", "strength"].includes(S.view.sizeBy)) S.view.sizeBy = "degree";
  if (S.view.colorBy !== "__none__" && !attributeKeys().includes(S.view.colorBy)) S.view.colorBy = "__none__";
  S.view.compFilter = null;
  S.rankBy = null;
  S.view.threshold = 0;
  S.view.hidden.clear();
  computeBasics();
  ...
  // say something useful instead of leaving the user with a blank canvas
  if (!nodeList.length) {
    toast(`No nodes came out of “${S.raw.name}”. Check that “${m.a}” and “${m.b}” are the right columns — ` +
      `rows where either is blank, or where both hold the same name, are skipped.`);
  } else if (!links.length) {
    toast(m.mode === "co"
      ? `${nodeList.length} nodes but no links: nothing in “${m.b}” is shared by two different “${m.a}” values.`
      : `${nodeList.length} nodes but no links — every row paired a name with itself.`);
  } else if (badWeights) {
    toast(`${badWeights.toLocaleString()} of ${rows.length.toLocaleString()} rows have no number in ` +
      `“${m.weightCol}”, so those edges weigh 0. Switch the weight back to “count of shared rows” if that is wrong.`);
  } else if (S.pairCount > 250000) {
    toast("This is a dense co-occurrence network — use the weight threshold in Style to thin it out.");
  }
}
...
function isHidden(n) {
  const cf = S.view.compFilter;
  if (cf != null && cf !== "all") {
    const want = cf === "largest" ? largestComponentId() : cf;
    if (n.comp !== want) return true;
  }
  const key = S.view.colorBy;
...
function largestComponentId() {
  if (S._bigComp != null) return S._bigComp;
  const size = new Map();
  for (const n of S.G.nodes) size.set(n.comp, (size.get(n.comp) || 0) + 1);
  let best = 0, bestN = -1;
  for (const [id, sz] of size) if (sz > bestN) { bestN = sz; best = id; }
  return (S._bigComp = best);
}
...
function paint(ctx, k) {
  ...
  for (const d of nodes) {
    ...
    ctx.stroke();
    if (S.picked.has(d.id) && !V.onlyPicked) {   // marked for export
      // (rings are pointless while Isolate is on: every visible node is marked)
...
/* ------------------------------------------------------- weak points ---- */
/** Articulation points (cut vertices): nodes whose removal breaks a component
    into pieces. For each one, report the sizes it would leave behind — the
    difference between a node that is merely busy and one the network depends on. */
function weakPoints(G) {
  const { N, head } = G;
  const adj = head.map(h => [...new Set(h.map(e => e[0]))]);
  const compOf = new Int32Array(N).fill(-1);
  const members = [];
  for (let s = 0; s < N; s++) {
    if (compOf[s] !== -1) continue;
    const q = [s], mem = []; compOf[s] = members.length;
    while (q.length) {
      const v = q.pop(); mem.push(v);
      for (const w of adj[v]) if (compOf[w] === -1) { compOf[w] = members.length; q.push(w); }
    }
    members.push(mem);
  }
  // pieces left inside a node's own component once it is removed
  const piecesWithout = (v) => {
    const mem = members[compOf[v]];
    const inSet = new Set(mem), seen = new Set([v]), out = [];
    for (const s of mem) {
      if (seen.has(s)) continue;
      const q = [s]; seen.add(s); let n = 0;
      while (q.length) {
        const u = q.pop(); n++;
        for (const w of adj[u]) if (inSet.has(w) && !seen.has(w)) { seen.add(w); q.push(w); }
      }
      out.push(n);
    }
  ...
  const out = [];
  for (let v = 0; v < N; v++) {
    if (adj[v].length < 2) continue;                 // a leaf can never be a cut point
    const p = piecesWithout(v);
    if (p.length > 1) out.push({
      node: G.nodes[v], pieces: p, was: members[compOf[v]].length,
      cutOff: p.slice(1).reduce((a, b) => a + b, 0),
      // how evenly it splits: 1 = perfectly in half, near 0 = shaves off a leaf
      evenness: +(p[1] / p[0]).toFixed(3)
    });
  }
  out.sort((a, b) => b.cutOff - a.cutOff || b.evenness - a.evenness);
  return out;
}
...
function graphStats(G, weighted) {
  ...
  // path statistics on the largest component (unweighted hops — the intuitive
  // reading). Components are numbered in discovery order, so the biggest one is
  // rarely number 0 — find it by size before measuring anything.
  let bigId = 0, bigSize = -1;
  const sizeOf = new Map();
  for (let i = 0; i < N; i++) sizeOf.set(seen[i], (sizeOf.get(seen[i]) || 0) + 1);
  for (const [id, sz] of sizeOf) if (sz > bigSize) { bigSize = sz; bigId = id; }
  const big = [];
  for (let i = 0; i < N; i++) if (seen[i] === bigId) big.push(i);
...
function renderStylePanel() {
  ...
  // an empty graph would otherwise hand the slider Infinity / -Infinity
  const wMin = ws.length ? minOf(ws) : 0, wMax = ws.length ? maxOf(ws) : 1;
  ...
  <div class="sect">
    <h3>Filter</h3>
    <div class="row">
      <label>Components${helpChip("components")}</label>
      <select id="s-comp">
        <option value="all">Show all ${S.G.componentCount} components</option>
        <option value="largest" ${V.compFilter === "largest" ? "selected" : ""}>Largest only (${componentSizes()[0]?.[1] ?? 0} nodes)</option>
        ${S.selected ? `<option value="sel" ${V.compFilter === S.selected.comp ? "selected" : ""}>The one containing ${esc(S.selected.label)}</option>` : ""}
      </select>
      <p class="hint">A network usually breaks into pieces that never touch. Narrowing to one lets the layout
        spread it out instead of squeezing it beside everything else.</p>
    </div>
    <div class="row">
      <label>Minimum edge weight <span class="pill" id="s-thlab">${fmt(V.threshold)}</span>${helpChip("threshold")}</label>
      <div class="slider">
        <input type="range" id="s-th" min="${wMin}" max="${wMax}" step="${((wMax - wMin) / 100) || 1}"
          value="${V.threshold}" ${ws.length ? "" : "disabled"}>
  ...
  on("#s-comp", "change", e => {
    V.compFilter = e.target.value === "all" ? null
      : e.target.value === "sel" ? (S.selected ? S.selected.comp : null)
        : "largest";
...
const EXPLAIN = {
  ...
  weak: `<b>What it finds.</b> Nodes the network cannot afford to lose. Remove one and its component falls into
    two or more pieces that no longer reach each other — a <em>cut vertex</em>, or articulation point.
    <b>Read it as</b> dependency rather than importance. A node with a hundred links inside one dense cluster
    can be removed without consequence; a node with two links can be the only thing joining two halves of the
    network. The "splits into" column is the answer to "what would be left".
    <b>Why it is often the sharpest result.</b> Centrality scores rank everyone on a smooth scale, which
    invites over-reading small differences. This is a yes/no structural fact: either the network survives
    without the node or it does not.
    <b>Watch out.</b> It is a statement about <em>this</em> record, not about the world. A person who looks
    indispensable may simply be the only one who happened to sign two surviving documents — absence of
    evidence for other routes is not evidence of dependence.`,
...
function renderAnalysisPanel() {
  ...
  p.innerHTML = `
  <div class="sect">
    <h3>Overview${helpChip("overview")}</h3>
    ${st ? `
  ...
  <div class="sect">
    ...
    <label class="chk"><input type="checkbox" id="a-weighted" ${S.useWeights ? "checked" : ""}>
      <span>Use edge weights (a heavier tie counts as a <i>shorter</i> distance)</span></label>${helpChip("weighted")}
  ...
  <div class="sect">
    <h3>Weak points <span class="pill">what breaks it</span></h3>
    <div id="a-weak"><button class="btn wide" id="a-weak-run">Find the nodes the network depends on</button></div>
    <div class="metric" id="card-weak" style="margin-top:9px">
      <button onclick="this.parentElement.classList.toggle('open')">
        <b>How this is measured</b><span class="caret">\u203a</span></button>
      <div class="why">${EXPLAIN.weak}</div>
    </div>
  </div>
  ...
  <div class="sect">
    <h3>Ranking${helpChip("ranking")}</h3>
    <div class="row">
  ...
  if (wk) wk.onclick = async () => {
    busy(true, "Testing every node\u2026"); await nextFrame();
    const rows = weakPoints(graphIndex());
    busy(false);
    const box = $("#a-weak");
    if (!rows.length) { box.innerHTML = `<p class="hint">No cut vertices \u2014 every component here survives
      the loss of any single node.</p>`; return; }
    box.innerHTML = `<p class="hint" style="margin-bottom:8px"><b>${rows.length}</b> node${rows.length === 1 ? "" : "s"}
        of ${S.G.nodes.length.toLocaleString()} would break their component in two.</p>
      <div class="tblwrap"><table><thead><tr><th>Node</th><th class="num">links</th>
        <th>splits into</th><th class="num">cut off</th></tr></thead><tbody>
      ${rows.slice(0, 60).map(r => `<tr data-id="${esc(r.node.id)}">
        <td>${esc(r.node.label)}</td><td class="num">${r.node.deg}</td>
        <td>${r.pieces.slice(0, 4).join(" + ")}${r.pieces.length > 4 ? " \u2026" : ""} <span style="color:#8a93a2">of ${r.was}</span></td>
        <td class="num">${r.cutOff}</td></tr>`).join("")}
      </tbody></table></div>
      <div class="btnrow" style="margin-top:8px"><button class="btn sm" id="a-weak-mark">Mark them all</button></div>`;
    $$("#a-weak tr[data-id]").forEach(tr => tr.onclick = () => focusNode(S.G.byId.get(tr.dataset.id)));
    $("#a-weak-mark").onclick = () => {
      rows.forEach(r => S.picked.add(r.node.id));
      renderSelectionBar(); renderExportPanel(); renderStatus(); draw();
    };
  };
  const hk = $("#a-homokey");
...
function renderExportPanel() {
  ...
  p.innerHTML = `
  <div class="sect">
    <h3>Picture of the current view${helpChip("picture")}</h3>
  ...
  <div class="sect">
    <h3>Marked group <span class="pill">${S.picked.size} selected</span>${helpChip("marked")}</h3>
    ${S.picked.size ? `
  ...
  <div class="sect">
    <h3>Session${helpChip("session")}</h3>
...
/* ========================================================= GUIDED TOUR === */
/* A short walk through the interface. Runs itself the first time someone opens
   the page, and is available afterwards from the Guide button. Each step can
   switch tabs and load the example first, so the thing being pointed at is
   actually on screen and populated. */
const TOUR = [
  {
    title: "A quick tour — about a minute",
    text: `This tool turns a spreadsheet into a network you can look at and measure. Nothing is uploaded:
      your file is read inside this browser tab and never leaves your computer.`,
    tip: "You can leave at any point with Skip or the Esc key, and come back later with the Guide button.",
    target: "#brand", tab: "data"
  },
  {
    title: "1 · Bring in a table",
    text: `Drop a CSV here, or click to pick one. Two shapes work: one row per <b>relationship</b>
      (person A, person B), or one row per <b>membership</b> (a person and a document, meeting or event) —
      the second is what most historical and archival spreadsheets look like.`,
    tip: "No file to hand? “Load example dataset” fills the tool with 350 people from Cairo Geniza legal documents.",
    target: "#drop", tab: "data"
  },
  {
    title: "2 · Say what the columns mean",
    text: `After the file loads, a dialog asks which column holds the nodes, which holds the link or the
      shared group, how to weigh the ties and what should appear when you hover. Nothing is guessed
      silently, so the network you get is the one you asked for.`,
    tip: "Everything there can be changed later with “Change column mapping”.",
    target: '[data-panel="data"]', tab: "data", demo: true
  },
  {
    title: "3 · Move around the picture",
    text: `Scroll to zoom, drag the background to pan. Hover a node or a link to see its details. Click a
      node to light up its immediate circle and dim everything else; click it again to release.
      Drag a node to pin it where you want it, double-click to let it go.`,
    tip: "Hold Shift and drag a box to mark a group of nodes — you can then download exactly that group.",
    target: "#stage", tab: "data"
  },
  {
    title: "4 · Find a specific person",
    text: `Type any part of a name here and pick from the list — the view flies to that node and selects it.
      Useful when you know who you are looking for and cannot see them in the tangle.`,
    target: "#searchwrap", tab: "data"
  },
  {
    title: "5 · Make it readable",
    text: `The Style tab is where a hairball becomes a picture. Colour by any category, size by any metric,
      choose how many names to show — and above all use the <b>minimum edge weight</b> slider, which hides
      weak ties until the structure appears.`,
    tip: "If the network is in several disconnected pieces, narrow to one under Components.",
    target: '#tabs button[data-tab="style"]', tab: "style"
  },
  {
    title: "6 · Measure it",
    text: `The Analysis tab computes centralities, communities and the graph-level summary. Every measure
      has a short explanation of what it counts, how to read it and what it hides — open the card to read it
      before trusting the number.`,
    tip: "“Weak points” is often the sharpest result: which nodes, if removed, break the network apart.",
    target: '#tabs button[data-tab="analysis"]', tab: "analysis"
  },
  {
    title: "7 · Take things away with you",
    text: `Export the current view as SVG for a paper or PNG for slides, the nodes with all their metrics as
      CSV, the whole graph for Gephi — or a session file that restores this exact figure later, positions
      and all.`,
    target: '#tabs button[data-tab="export"]', tab: "export"
  },
  {
    title: "Wherever you see a ?",
    text: `Small question marks sit next to the controls that deserve a sentence of explanation. Hover one
      and it appears. That is the whole interface — go and break something.`,
    tip: "The Guide button in the top corner replays this tour whenever you want.",
    target: "#guide", tab: "data"
  }
];

let tourAt = -1;
const TOUR_KEY = "network-explorer/seen-tour";
const tourEl = {
  root: $("#tour"), hole: $("#tour-hole"), card: $("#tour-card"),
  title: $("#tour-title"), text: $("#tour-text"), tip: $("#tour-tip"), dots: $("#tour-dots")
};

function startTour() { tourAt = -1; tourEl.root.classList.add("on"); tourStep(1); }
function endTour() {
  tourEl.root.classList.remove("on");
  try { localStorage.setItem(TOUR_KEY, "1"); } catch (e) { /* private mode — no matter */ }
}
function tourStep(delta) {
  ...
  if (st.demo && !S.G) loadDemo();
  if (st.tab) showTab(st.tab);

  tourEl.title.textContent = st.title;
  tourEl.text.innerHTML = st.text;
  tourEl.tip.innerHTML = st.tip || "";
  tourEl.tip.style.display = st.tip ? "block" : "none";
  tourEl.dots.innerHTML = TOUR.map((_, i) => `<i class="${i === tourAt ? "on" : ""}"></i>`).join("");
  $("#tour-back").style.visibility = tourAt ? "visible" : "hidden";
  $("#tour-next").textContent = tourAt === TOUR.length - 1 ? "Done" : "Next";

  // let the panel finish rendering before measuring where to point
  requestAnimationFrame(() => {
    const el = $(st.target);
    const r = el ? el.getBoundingClientRect() : { top: innerHeight / 2, left: innerWidth / 2, width: 0, height: 0 };
    const pad = 6;
    Object.assign(tourEl.hole.style, {
      top: (r.top - pad) + "px", left: (r.left - pad) + "px",
      width: (r.width + pad * 2) + "px", height: (r.height + pad * 2) + "px"
    });
    const cw = 340, ch = tourEl.card.offsetHeight || 220;
    let left = r.right + 18, top = r.top;
    if (left + cw > innerWidth - 14) left = r.left - cw - 18;      // flip to the other side
    if (left < 14) left = Math.min(innerWidth - cw - 14, r.left + 14);
    if (top + ch > innerHeight - 14) top = Math.max(14, innerHeight - ch - 14);
    tourEl.card.style.left = left + "px";
    tourEl.card.style.top = Math.max(14, top) + "px";
  });
}
$("#tour-next").onclick = () => tourStep(1);
$("#tour-back").onclick = () => tourStep(-1);
$("#tour-skip").onclick = endTour;
$("#guide").onclick = startTour;
$("#tour-veil").onclick = endTour;
...
window.addEventListener("keydown", e => {
  if (e.key === "Escape") {
    if (tourEl.root.classList.contains("on")) return endTour();
    closeModal();
    if (S.selected) { S.selected = null; renderAnalysisSelection(); draw(); }
  }
  if (tourEl.root.classList.contains("on")) {
    if (e.key === "ArrowRight" || e.key === "Enter") { e.preventDefault(); tourStep(1); }
    if (e.key === "ArrowLeft") { e.preventDefault(); tourStep(-1); }
    return;
  }
...
// first visit: offer the tour rather than dropping someone into an empty canvas
try {
  if (!localStorage.getItem(TOUR_KEY)) setTimeout(startTour, 550);
} catch (e) { /* storage unavailable — skip the auto-run, the Guide button still works */ }
/* END */
